...
class CloudDeviceCommercialGatewayTest {
    ...
    fun `fixture quotes cover no campaign invalid expired unavailable production and zero ratio`() =
        runBlocking {
            ...
            assertEquals(6_500, production.quote.originalAmountCents)
            assertEquals(3_900, production.quote.finalAmountCents)
    ...
    fun `permanent pro check detects revocation immediately without license refresh`() =
        runBlocking {
            ...
            val activeRead = fixture.gateway.checkEntitlementRead(fixture.now)
            assertTrue(activeRead.remoteConfirmed)
            ...
            val fallbackRead = fixture.gateway.checkEntitlementRead(fixture.now)
            assertEquals(
                CommercialAccessRefreshResult.Ready(EntitlementState.Pro),
                fallbackRead.value
            )
            assertFalse(fallbackRead.remoteConfirmed)
            assertTrue(
    ...
    @Test
    fun `confirmed device mismatch stays denied through recovery failure and reopens only after persistence`() =
        runBlocking {
            val fixture = FixtureHarness()
            val initial = fixture.gateway.queryEntitlement(fixture.now)
                as EntitlementQueryResult.Ready
            ...
            fixture.transport.entitlementScenario = DebugEntitlementScenario.DEVICE_KEY_MISMATCH
            fixture.transport.recoveryScenario = DebugRecoveryScenario.NETWORK_ERROR

            val offlineRecovery = fixture.gateway.checkEntitlementRead(fixture.now)
            assertEquals(
                CommercialAccessRefreshResult.Failure(CommercialFailure.DEVICE_MISMATCH),
                offlineRecovery.value
            )
            assertTrue(offlineRecovery.remoteConfirmed)
            assertTrue(
                fixture.store.read(SecureCommercialRecord.LICENSE) is SecureStoreReadResult.Value
            )
            assertTrue(
                fixture.store.read(SecureCommercialRecord.DEVICE_TOKEN) is
                    SecureStoreReadResult.Value
            )
            assertTrue(
                fixture.store.read(SecureCommercialRecord.ENTITLEMENT_RECHECK_PENDING) is
            ...
            val rejectedRecovery = fixture.gateway.checkEntitlementRead(fixture.now)
            assertEquals(
                CommercialAccessRefreshResult.Failure(CommercialFailure.DEVICE_MISMATCH),
                rejectedRecovery.value
            )
            assertTrue(rejectedRecovery.remoteConfirmed)

            fixture.transport.recoveryScenario = DebugRecoveryScenario.SAME_DEVICE
            val recovered = fixture.gateway.checkEntitlementRead(fixture.now)
            assertEquals(
                CommercialAccessRefreshResult.Ready(EntitlementState.Pro),
                recovered.value
            )
            assertTrue(recovered.remoteConfirmed)
            val claims = DeviceCommerceLicenseClaimsParser.parse(
                SignedLicenseEnvelopeCodec.decode(
                    (fixture.store.read(SecureCommercialRecord.LICENSE)
                        as SecureStoreReadResult.Value).bytes
            ...
            assertEquals(CommercialTier.PRO, claims.tier)
            assertEquals(LicenseValidity.PERMANENT, claims.validity)
            assertEquals(2, claims.deviceKeyVersion)
            assertEquals(
                SecureStoreReadResult.Missing,
                fixture.store.read(SecureCommercialRecord.ENTITLEMENT_RECHECK_PENDING)
            )
