...
class CloudDeviceCommercialGateway(
    ...
    override suspend fun checkEntitlement(
        nowEpochMs: Long
    ): CommercialAccessRefreshResult = checkEntitlementRead(nowEpochMs).value
    ...
    override suspend fun checkEntitlementRead(
        ...
        return CommercialEntitlementReadResult(
            value = result.toCommercialRefreshResult(),
            remoteConfirmed = result.remoteConfirmed
        )
    }
    ...
    override suspend fun queryEntitlement(nowEpochMs: Long): EntitlementQueryResult =
        ...
        // endpoint.
        queryEntitlementRead(nowEpochMs, forceRemote = true).value
    ...
    override suspend fun queryEntitlementRead(
        ...
        val query = when (val refreshed = accessResult) {
            is AccessRefreshResult.Failure -> EntitlementQueryResult.Failure(refreshed.reason)
            is AccessRefreshResult.Ready -> {
        ...
        return CommercialEntitlementReadResult(
            value = query,
            remoteConfirmed = accessResult.remoteConfirmed
        )
    }
    ...
    private fun AccessRefreshResult.toCommercialRefreshResult(): CommercialAccessRefreshResult =
        when (this) {
            is AccessRefreshResult.Ready -> CommercialAccessRefreshResult.Ready(entitlement)
            is AccessRefreshResult.Failure -> CommercialAccessRefreshResult.Failure(reason)
        }
    ...
    private suspend fun checkAccessOnce(nowEpochMs: Long): AccessRefreshResult {
        ...
        val identity = runCatching { identityProvider.loadOrCreate() }.getOrElse {
            return AccessRefreshResult.Failure(
        ...
        return when (val check = checkRemoteEntitlement(identity, expectedKeyVersion)) {
            ...
            EntitlementCheckStatus.DeviceKeyMismatch -> {
                resolveRemoteEntitlement(
                    recoverAfterDeviceKeyMismatch(nowEpochMs),
                    // The cloud has already proved that this credential is
                    // attached to a different device key.  A recovery
                    // transport failure must therefore stay denied instead
                    // of reopening the old local entitlement.
                    localFallbackEntitlement = null,
                    identity = identity,
            ...
            is EntitlementCheckStatus.Failure -> {
                ...
                if (localFallbackEntitlement != null && check.allowLocalFallback) {
                    AccessRefreshResult.Ready(
                        localFallbackEntitlement,
                        identity,
                        remoteConfirmed = false
                    )
                } else {
                    AccessRefreshResult.Failure(
                        check.reason,
                        remoteConfirmed = !check.allowLocalFallback
                    )
                }
            }
        }
    ...
    /**
     * A key-mismatch response is an authoritative boundary.  Recovery may
     * replace the local credential only after it has completed successfully;
     * transient or incomplete recovery results must not use the old license.
     */
    private suspend fun recoverAfterDeviceKeyMismatch(nowEpochMs: Long): RemoteEntitlement =
        when (val recovered = recoverForActiveCheck(nowEpochMs)) {
            is RemoteEntitlement.Ready,
            RemoteEntitlement.Expired -> recovered
            RemoteEntitlement.RecoveryRequired -> RemoteEntitlement.RecoveryRequired
            is RemoteEntitlement.Failure -> when (recovered.reason) {
                // The recovery path already cleared local credentials for a
                // confirmed cloud revocation; preserve that more specific
                // terminal reason for the shared coordinator.
                CommercialFailure.ENTITLEMENT_REVOKED -> recovered
                else -> RemoteEntitlement.RecoveryRequired
            }
        }
    ...
    private fun resolveRemoteEntitlement(
        ...
        RemoteEntitlement.RecoveryRequired -> {
            // Keep a durable retry hint while the old credential remains in
            // storage, but never expose it as an authorized fallback.
            licenseRepository.markRecheckPending(nowEpochMs)
            AccessRefreshResult.Failure(
    ...
    private sealed interface AccessRefreshResult {
        ...
        data class Ready(
            ...
            override val remoteConfirmed: Boolean = true
        ) : AccessRefreshResult
