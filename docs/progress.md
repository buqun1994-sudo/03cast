...
## 2026-09-19 网络视频完整手势与下一条能力链（代码完成，待用户主测）

1. 已替换旧的本地队列上下滑：上滑与控制栏“下一条”按钮共用唯一播放意图，仅对已知时长、可跳转的网络视频生效；按“发送端已知下一项 -> AirPlay DACP `nextitem` -> 当前协议自然终态投影”的顺序选择最强能力。真机日志确认旧 `250 ms` 方案每次先为远端 MP4 尾部重新缓冲约 `2.5～2.7 s`，实际尾段约 `0.38 s`，随后手机在约 `0.27～0.34 s` 内发送 Stop，慢点和失败均不由 `1500 ms` 窗口造成。现已完全删除尾段 seek：DLNA 在用户松手时立即投影 `position=duration + STOPPED`，AirPlay 立即投影 UxPlay 自然结束使用的 `duration=-1` 完成哨兵；该哨兵在原生层跨播放器自身的 HLS 分片连接保持，只由真实新视频或无响应恢复解除。`1500 ms` 只关联 Stop 与新媒体，不参与触发延迟；新媒体到达立即复用播放器。GENA 改为严格串行，消除强制 seek 产生的密集状态乱序。下滑不执行上一条，也不显示接收端队列边界提示。
...
4. 已验证：手势策略、Media3 控制桥、网络队列、发送端交接窗口和 DLNA SOAP / 终态投影直接相关 JVM 用例全部通过；全量 Debug JVM 共执行 `225` 条，`222` 条通过，仅仓库既有 `IcarThemeColorPaletteTest` 三条主题色断言失败。`externalNativeBuildDebug`、`lintDebug` 与 `assembleDebug` 通过，项目就绪检查、Skill 快检、版本检查和 `git diff --check` 通过；模板快检按预期拒绝当前已初始化的 03投屏具体项目态，不属于本轮回归。共享 03 APP 登记 Guard 仍因 Cloud 登记停留在 `1.0.6 (7)`、旧 HEAD 与 clean 快照，而本仓库已为 `1.0.8 (9)` 且处于施工工作树而失败；本轮未越权修改共享登记。Debug APK 已通过统一枚举入口重新覆盖安装到 `192.168.0.203:5555 / S56_HQX / Android SDK 28 / 1920x1080`，实际安装版本为 `1.0.8-icar03-test (9)`；安装脚本未执行启动命令，安装后检测到接收进程已由车机系统或既有服务恢复运行。
5. 协议边界已确认：DLNA 只有发送端提供 `SetNextAVTransportURI` 才有确定的标准下一项，现已补齐标准 `Next` 动作与动态能力声明；AirPlay HLS 可由 `playlistInsert` 提供队列，并可在发送端暴露 DACP 时尝试真正的 `nextitem / previtem`。接收端只有当前 URL 时无法推导抖音推荐流；协议终态是无尾部加载的兼容请求，手机应用仍可合法不提供下一项。产品仍按既定决定不提供下滑上一条，协议层的 `previtem` 不接入界面。
6. 待用户主测：分别以安卓 DLNA 和苹果手机实际协商出的投屏协议验证上滑、下一条按钮、暂停态下一条、左右横滑、隐藏态仅显示进度、两个设置开关及全部淡入淡出；下一条重点记录是否命中真实队列 / DACP / `protocol-end`、终态 GENA 序号、Stop 是否被交接窗口延迟以及新媒体是否在窗口内到达。
