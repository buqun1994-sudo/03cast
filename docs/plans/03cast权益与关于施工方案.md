...
3. `Pro` 复用 03 歌词的 `ic_commercial_pro_crown` 皇冠资源，按正文实际字面高度裁定可见笔画后通过内联 `ReplacementSpan` 与“Pro 权益生效中 · 永久”组成同一行富文本；该权益行从主等待内容中抽离，固定放在“设置”入口上方；不显示“当前可以投屏”或“查看权益”。撤权 `Error(ENTITLEMENT_REVOKED)` 明确显示“权益已撤销”和获取 Pro 入口，不显示“当前无法确认投屏权益”“查看权益”或“重新确认”；其它错误继续保留重新确认和查看权益动作。等待页所有权益状态共用两个固定锚点：主状态组“图标 -> 03投屏 -> 等待投屏”固定在中上部，底部辅助组“权益信息 / 广告 -> 设置”固定在偏下部；权益状态变化只替换辅助组内容。
...
| `Expired` | 拒绝 | 显示购买 Pro |
| `Error` | 拒绝 | 普通错误显示重新确认；`ENTITLEMENT_REVOKED` 显示撤权结果并提供获取 Pro 入口 |
