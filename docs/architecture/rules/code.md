...
6. 协议 Stop、AirPlay 视频 Stop 或协议权威的输出拆除必须立即失效当前租约、释放输出并发布 `REMOTE_DISCONNECTED`，不得依据远程断开事件自动改变 Activity 的标准浮窗 / 全屏模式。网络媒体真实自然结束时，必须由 Media3 先产生终点快照和 `STATE_ENDED`，再保留当前媒体投影最多 `1500 ms` 接收已经在途的下一项；新媒体项到达即继续，超时则立即释放输出并回到等待态。该自然结束等待只属于播放器队列；发送端在窗口内发来 Stop 时，Stop 本身必须立即完成当前输出，随后到达的新媒体按正常新会话接管。协议适配器不得脱离播放器单独伪造终态，也不得拦截 Stop 等待新媒体。用户关闭活动态会话必须发布 `USER_REQUEST`；全屏等待态的窗口返回必须由用户可见的“退出全屏”动作触发。
...
1. 触发条件：协议发送端会提前提供下一条网络媒体、接收端需要请求发送端自动连播，或播放器存在多个本地 HLS 请求并发读取。
2. 应采取动作：由接收端队列维护待播顺序、当前项、上一项、下一项、准备态、重试次数和自然结束窗口；协议适配器只提交媒体事实。每个 HLS 缓存项必须生成不可复用的本地 URL 命名空间，HTTP 请求按该命名空间或带请求编号的远端 URL 路由到所属项，禁止用进程级 `current_video` 作为唯一键。
3. 车机触发的“下一条”只有一个 Router 入口，并由 `NextVideoPolicy` 作为按钮能力和实际执行共用的纯决策真值，按两条确定分支执行：若统一队列已经存在发送端提供的下一项，立即调用 `NetworkPlaybackQueue.next()` 选择它；否则要求当前网络视频可跳转且时长已知，通过与用户手动拖动进度条完全相同的协议 Seek 主链，一次性跳转到 `max(0, durationMs - 1000ms)`。上滑和控制栏按钮不得各自实现分支。不得发送 DACP `nextitem`、设置确认 / 回退计时、精确 seek 到总时长、调用播放器专用结束命令，或由 DLNA / AirPlay 适配器伪造终态。近尾 Seek 后的最后一秒由播放器自然播放，之后若产生 `STATE_ENDED`，再进入既有自然结束队列。UxPlay 的 `duration=-1` 仍只表示播放信息不可用，禁止用作人工结束哨兵；DACP 继续只服务 AirPlay 音频控制。
4. 播放器只执行队列投影：更新 `MediaItem` 列表时保留当前播放器和输出表面，选择时按稳定项 ID 定位；队列状态变更带代次，迟到的结束、错误、准备回调不能改变新当前项。同一项 ID 的来源地址或 MIME 改变时必须创建新代次并重新选择，只有展示信息变化才原位更新。GENA 的同一事件流必须按 `SEQ` 产生顺序串行投递，禁止并发 socket 让 `PLAYING / TRANSITIONING / STOPPED` 乱序到达；网络错误按 `250 / 750 / 1500 ms` 退避重试三次。
5. 验证方式：队列 reducer 单测覆盖 10 项上限、当前项保护、Next 替换、前后边界、失败跳过、迟到回调、重试退避和自然结束窗口；源码契约锁定上滑和控制栏按钮调用同一入口、已知下一项优先调用 `NetworkPlaybackQueue.next()`、无下一项时目标严格为 `durationMs - 1000ms` 且复用手动拖动的内部 Seek 入口，同时确认播放器专用结束命令、协议终态投影、DACP 视频下一条和能力回退计时器均不存在；SOAP 单测继续覆盖 `SetNextAVTransportURI -> Next` 与动态 `CurrentTransportActions`；原生编译验证人工 `duration=-1` JNI 与受保护哨兵字段已删除。真机日志分别验证 `provided-next` 立即换片，以及 `near-end-seek` 的目标位置与手动拖到同一位置所得协议状态一致。
6. 适用边界：DLNA 只有发送端预先调用 `SetNextAVTransportURI` 时才存在确定的标准下一项；AirPlay 只有发送端提供 `playlistInsert` 时接收端才已知下一项。接收端无法从当前媒体 URL 推导抖音推荐流的上一条或下一条；无下一项时的近尾 Seek 只复用发送端已经支持的定位行为，手机应用是否在随后自然结束时生成下一项仍由发送端决定。本规则不改变镜像 RTP、AirPlay 音频 DACP 或 DLNA 发送端主动 `Next` 的既有行为，也不重新启用下滑上一条交互。
...
1. 触发条件：生命周期权益查询尚未返回时，用户在同一 Activity 内切换权益首页、订单页或二维码页。
...
## 14. Android VPN 共存与局域网投屏网络选择规则

1. 触发条件：车机存在 `TRANSPORT_VPN`，而 DLNA / SSDP / AirPlay 接收端仍必须在发送端所在局域网被发现或访问。
2. 应采取动作：网络适配层使用带 `NET_CAPABILITY_NOT_VPN` 的 `NetworkRequest` 枚举物理网络，优先选择带默认路由的 Wi‑Fi，再按 Wi‑Fi、Ethernet、其它物理网络和验证状态评分；同时排除 `tun*`、`ppp*`、`rmnet*`、`ccmni*`、`wwan*`、`dummy*` 与 `lo` 等虚拟接口。DLNA HTTP、SSDP 组播和描述地址绑定选中的物理地址 / 接口，禁止用 `activeNetwork` 直接决定接收地址。
3. 媒体请求只能在连接级绑定：发送端提供的局域网媒体地址和图片地址通过 `Network.openConnection` 走当前物理网络；公网 CDN 地址及其重定向沿用系统默认网络。禁止调用 `bindProcessToNetwork`，禁止改变其它模块或朋友 VPN 的路由；发现、权益、配置和支付请求不引入媒体网络选择逻辑。
4. VPN 启停本身不得重启接收器；只有物理地址 / 接口或物理网络对象变化时才关闭旧 DLNA / mDNS 资源并重新绑定。选中的地址、接口和是否使用接口回退必须写入可观测日志，但不得记录媒体内容或持久化发送端身份。
5. 验证方式：JVM 单测覆盖 VPN / 隧道地址被排除、默认路由在多个物理 Wi‑Fi 中胜出、仅有隧道时无可用地址；目标车机诊断确认 VPN 开启时 `8200` 监听在物理 Wi‑Fi、SSDP 可发现“03投屏”，并确认 VPN 切换不导致端口或接收进程重启。真实抖音 / 哔哩哔哩搜索和播放由用户手测确认。
6. 适用边界：本规则只约束本项目 Android 9 车机的 DLNA / AirPlay 接收和网络媒体；商业 API、支付、其它应用网络以及公网媒体的 VPN 策略不由接收器改写。
