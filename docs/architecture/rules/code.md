...
6. 媒体自然结束、协议 Stop、AirPlay 视频结束或协议权威的输出拆除必须立即失效当前租约、释放输出并发布 `REMOTE_DISCONNECTED`，不得设置会话连续性等待，也不得依据远程断开事件自动改变 Activity 的标准浮窗 / 全屏模式。AirPlay HLS 队列项自然结束时，若等待发送端的 `playlistInsert` 换片动作，允许仅对该自然结束路径保留当前租约最多 `1500 ms`；换片回调到达即取消等待，超时仍按立即断开处理。显式协议 Stop、用户关闭活动态会话和其它媒体结束路径不得使用该窗口，用户关闭必须发布 `USER_REQUEST`；全屏等待态的窗口返回必须由用户可见的“退出全屏”动作触发。
...
1. 触发条件：协议发送端会提前提供下一条网络媒体，或播放器存在多个本地 HLS 请求并发读取。
...
3. 播放器只执行队列投影：更新 `MediaItem` 列表时保留当前播放器和输出表面，选择时按稳定项 ID 定位；队列状态变更带代次，迟到的结束、错误、准备回调不能改变新当前项。同一项 ID 的来源地址或 MIME 改变时必须创建新代次并重新选择，只有展示信息变化才原位更新。自然结束最多等待 `1500 ms` 接收下一项，网络错误按 `250 / 750 / 1500 ms` 退避重试三次，显式 Stop 立即结束。
4. 验证方式：队列 reducer 单测覆盖 10 项上限、当前项保护、Next 替换、前后边界、失败跳过、迟到回调、重试退避和自然结束窗口；原生单测覆盖两个 HLS 命名空间、请求编号归属、旧项销毁后的 URL 不再匹配；真实 DLNA / AirPlay 连续短视频由目标车机人工验收。
5. 适用边界：这是网络媒体接收队列规则，不改变镜像 RTP 的帧时序，也不要求发送端必须实现 DLNA `SetNextAVTransportURI` 或 AirPlay `playlistInsert`。
