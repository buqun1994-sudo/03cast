...
6. 媒体自然结束、协议 Stop、AirPlay 视频结束或最后一条 AirPlay 连接销毁，不得直接依据普通 `WAITING` 状态让 Activity 回浮窗；Router 必须先保留当前租约最多 `2 s`，同协议新媒体 URI、Play、轮询或新连接到达即取消待断开任务，确认超时后才发布带 `REMOTE_DISCONNECTED` 原因的结束事件。用户关闭活动态会话必须发布 `USER_REQUEST`，不能被换片逻辑延迟。
7. 验证方式：状态机单测覆盖新会话、同协议新会话、当前租约复用、旧租约失效和停止失效；DLNA -> AirPlay 接管、短视频自动下一条保持全屏、真实远程断开约 `2 s` 后回浮窗、停止后再次启动与端口恢复由用户手测或明确授权的诊断脚本确认。
