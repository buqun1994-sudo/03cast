...
6. 协议 Stop、AirPlay 视频 Stop 或协议权威的输出拆除必须立即失效当前租约、释放输出并发布 `REMOTE_DISCONNECTED`，不得依据远程断开事件自动改变 Activity 的标准浮窗 / 全屏模式。网络媒体自然结束时必须先发布播放器的终点快照并保留当前媒体投影，仅该路径允许最多 `1500 ms` 接收已经在途的下一项；新媒体项到达即继续，超时则立即释放输出并回到等待态。唯一例外是接收端刚发布了“请求发送端前进”的协议终态：手机常按 `Stop -> 新媒体` 完成交接，因此该 Stop 只在同一 `1500 ms` 关联窗口内延迟销毁，新媒体到达立即完成交接，只有 Stop 而无新媒体才在窗口结束时释放。用户关闭活动态会话和窗口外的显式 Stop 不得使用该窗口，用户关闭必须发布 `USER_REQUEST`；全屏等待态的窗口返回必须由用户可见的“退出全屏”动作触发。
...
1. 触发条件：协议发送端会提前提供下一条网络媒体、AirPlay 网络视频会话暴露发送端遥控凭证，或播放器存在多个本地 HLS 请求并发读取。
...
3. “下一条”播放意图按能力强弱执行：队列已有下一项时立即选择；AirPlay 会话暴露有效 DACP 端点时发送 `nextitem`，HTTP 成功只代表手机收到命令，必须以新媒体项到达作为成功证据；仍无新项时由当前协议适配器发布与自然结束相同的终态投影，DLNA 暴露 `position=duration + STOPPED`，AirPlay 暴露 UxPlay 自然结束使用的 `duration=-1` 完成哨兵。终态必须在用户松手或点击时立即发布，`1500 ms` 只用于关联随后到达的 Stop 与新媒体，不得作为触发延时。AirPlay 完成哨兵必须跨播放器自身的 HLS 分片连接保持，只允许真实新视频开始或窗口无响应恢复真实播放信息时解除，禁止把任意新 TCP 连接误判为新视频。该兼容路径不得 seek 远程媒体、请求尾部数据或重建解码器；媒体像素对手机不可见，导航控制不得借媒体数据面传递。DACP 服务发现必须覆盖当前会话，服务消失立即清除端点，旧解析回调不得复活失效地址。
...
5. 验证方式：队列 reducer 单测覆盖 10 项上限、当前项保护、Next 替换、前后边界、失败跳过、迟到回调、重试退避、终点投影和自然结束窗口；发送端前进窗口单测覆盖无响应、Stop 延迟与新媒体立即完成；SOAP 单测覆盖 `SetNextAVTransportURI -> Next`、动态 `CurrentTransportActions` 和终态媒体保留；原生编译验证 AirPlay 完成投影 JNI 与受保护哨兵字段一致。真机日志必须能按序观察终态投递、Stop 延迟及新媒体完成，并确认终态发布前后出现的 HLS 分片连接不会提前清除完成哨兵。真实 DLNA / AirPlay 连续短视频和 AirPlay DACP 是否被具体手机应用接受由目标车机人工验收。
6. 适用边界：DLNA 只有发送端预先调用 `SetNextAVTransportURI` 时才存在确定的标准下一项；AirPlay 只有发送端提供 `playlistInsert` 或接受 DACP 时才存在确定切换能力。接收端无法从当前媒体 URL 推导抖音推荐流的上一条或下一条；协议终态只消除不必要的数据加载并复现发送端可观察的结束事实，不能强制拒绝自动连播的手机应用生成下一项。本规则不改变镜像 RTP 的帧时序，也不重新启用下滑上一条交互。
