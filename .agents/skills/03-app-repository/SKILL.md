...
description: 03 APP 客户端仓库入口；在 03cast（03投屏）的包名、namespace、版本、签名、许可证、远端或发布产物任务中调用共享的 03 APP 管家规则。
---

# 03 APP 仓库入口

本仓库登记为 `productId=03cast`，显示名称为“03投屏”。这是路由 skill，不复制身份规则正文。
...
3. 本产品固定包名和 namespace 为 `com.ninepointnine.desktopcast`，`licenseMode=device-license`；必须与 03lyrics 隔离签名、许可证、设备身份和产物。旧包名 `com.tcrrry.desktopcast` 一律阻断。
